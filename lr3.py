...
for i in range(7):
   ...
   Aj += Am
   C_ost_lst_2.append(round(C_ost, 2)) #что это обозначает? это остаток, который остается после вычета суммы амортизации, тоесть остаток от стоимости нового оборудования Оценка: 3/5 тк это добавление округленного остатка в список
